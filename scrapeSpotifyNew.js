...
async function scrapeArtist(browser, url) {
  ...
  try {
    ...
    // Optimizamos carga: solo esperamos a que el DOM esté listo
    await page.goto(url + "?locale=en", {
      waitUntil: "domcontentloaded",
      timeout: 45000,
    });

    // 🎤 Nombre (Usando h1 o meta title)
    try {
    ...
    // 🖼️ Imagen artista Historia y Background (selectores actuales como fallback)
    try {
      const historyStyle = await page.locator('.J8g7rZ2MDknxmiYP.GUhu8mqqIMM0sZzG').first().getAttribute("style", { timeout: 2000 }).catch(() => null);
      if (historyStyle) {
        const match = historyStyle.match(/url\("(.*?)"\)/);
        artistImageHistory = match ? match[1] : null;
      }

      const bgStyle = await page.locator('.jiWxWueoicolJZnS.jX9OuHoZE8EC2SYi').first().getAttribute("style", { timeout: 2000 }).catch(() => null);
      if (bgStyle) {
    ...
    // 🎵 Tracks (Solo si es necesario)
    try {
      // Intentar mostrar más canciones si el botón existe
      const moreSongsBtn = page.locator('.sL0wneoReggMaCXb').first();
      if (await moreSongsBtn.isVisible({ timeout: 2000 })) {
        await moreSongsBtn.click();
      }
      ...
      for (let i = 0; i < Math.min(10, tracks.length); i++) {
        ...
        try {
          title = await track.locator('[data-testid="internal-track-link"]').textContent({ timeout: 1000 });
          const href = await track.locator("a").first().getAttribute("href", { timeout: 1000 });
      ...
      console.log("⚠️ Error procesando canciones");
    }

    console.log(`✅ ${name || "Sin nombre"} (${listeners || 0} oyentes)`);

  } catch (error) {
    console.error("❌ Error en artista:", url, error.message);
  } finally {
    await page.close();
  }
...
// 🔹 Main loop
async function run() {
  console.log("🚀 Iniciando Scraper Optimizado...");
  const browser = await chromium.launch({ 
    headless: true,
    args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage'] // Mejor estabilidad en Docker/Low-RAM
  });
  ...
  try {
    ...
    const rankedResults = results
      .filter(a => a.name) // Filtrar fallidos
...
function saveResults(data) {
  const filePath = "./spotify-data-new.json";
  let existingData = [];
